...
public class SubcriptionController {
	...
	@GetMapping(path = { "/subscription/weekly"})
	public String subscriptionWeekly(final HttpServletRequest request, final HttpServletResponse response, final HttpSession session) throws Exception {
		CommonDTO commonDTO=new CommonDTO(10.00,SubscriptionType.WEEKLY,"TUESDAY","01/09/2021","01/11/2021");

		return subscriptionService.performSubscription(commonDTO, session, request);
	}
	
	@GetMapping(path = { "/subscription/monthly"})
	public String subscriptionMonthly(final HttpServletRequest request, final HttpServletResponse response, final HttpSession session) throws Exception {
		CommonDTO commonDTO=new CommonDTO(10.00,SubscriptionType.MONTHLY,"23","01/09/2021","01/11/2021");

		return subscriptionService.performSubscription(commonDTO, session, request);
	}
	
	@GetMapping(path = { "subscription/monthly/exceed"})
	public String subscriptionExceedMonthly(final HttpServletRequest request, final HttpServletResponse response, final HttpSession session) throws Exception {
